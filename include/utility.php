...
$account_type = array("Current Assets","Customer/Parties","Capital");
$customer_type = array("Debit","Credit");
function admin_logo(){
...
function getProductType($value){
	if( $value == 1 ) {
		...
		return "Sale Item / Purchase Item";
	}
}
...
function get_order( $id, $module = 'sales' ) {
	global $dblink;
	$order= dofetch(doquery("select * from ".$module." where id='".$id."'", $dblink));
	$payment_account_id = "";
	$payment_amount = 0;
	if( !empty( $order[ "transaction_id" ] ) ) {
		...
		if( numrows( $transaction ) > 0 ) {
			...
			if( $module == "sales" ) {
				...
				$payment_account_id = $transaction[ "reference_id" ];
			}
	...
	$order = array(
		"id" => $id,
		"token_number" => get_token_number( $order, $module ),
	...
	$rs = doquery( "select a.*, b.title from ".$module."_items a inner join items b on a.item_id = b.id where ".$module."_id='".$id."' order by b.sortorder desc", $dblink );
	if( numrows($rs) > 0 ) {
		while( $r = dofetch( $rs ) ) {
			...
			$order["items"][] = array(
				"id" => $r[ "id" ],
				"item_id" => $r[ "item_id" ],
				"packing" => (float)$r["packing"],
				"rate" => $r["rate"],
				"unit_price" => (float)$r[ "unit_price" ],
				"quantity" => (float)$r[ "quantity" ],
				"less_weight" => (float)$r[ "less_weight" ],
...
function get_last_closing_dt( $dt = "" ){
	global $dblink;
	if( empty( $dt ) ) {
		$dt = date( "Y-m-d H:i:s" );
	}
	...
	if( !isset( $GLOBALS["lastclosing"][ $date ] ) ) {
		$rs = doquery( "select * from closing_activity where datetime <= '".$dt."' order by datetime desc limit 0,1", $dblink );
		if( numrows( $rs ) > 0 ) {
			$r = dofetch( $rs );
			$dt = $r[ "datetime" ];
		}
		else{
			$dt = date( "Y-m-d", strtotime( "-1 day", strtotime( $date )))." ".get_config( "closing_hour" ).":00";
		}
		$GLOBALS["lastclosing"][ $date ] = $dt;
	}
	return $GLOBALS["lastclosing"][ $date ];
}
function get_next_closing_dt( $dt = "" ){
	global $dblink;
	if( empty( $dt ) ) {
		$dt = date( "Y-m-d H:i:s" );
	}
	...
	if( !isset( $GLOBALS["nextclosing"][ $date ] ) ) {
		...
		if( numrows( $rs ) > 0 ) {
			$r = dofetch( $rs );
			$dt = $date." ".get_config( "closing_hour" ).":00";
		}
		else{
			$dt = $date." ".get_config( "closing_hour" ).":00";
		}
...
function get_customer_balance( $customer_id, $dt = 0 ){
	...
	if( numrows( $customer ) > 0 ) {
		...
		$sql="select sum(amount) as amount from (select concat( 'Sale #', id) as transaction, (select sum(total_price) from sales_items where sales_id = sales.id)-discount as amount from sales where customer_id = '".$customer_id."' and datetime_added <='".$dt."' union select concat( 'Sale Return #', id) as transaction, discount-(select sum(total_price) from sales_return_items where sales_return_id = sales_return.id) as amount from sales_return where customer_id = '".$customer_id."' and datetime_added <='".$dt."' union select concat( 'Payment #', id) as transaction, if(type=0,-1,1)*amount as amount from customer_payment where customer_id = '".$customer_id."' and datetime_added <='".$dt."') as transactions";
...
function get_supplier_balance( $supplier_id, $dt = 0 ){
	...
	if( numrows( $supplier ) > 0 ) {
		...
		$sql="select sum(amount) as amount from (select concat( 'Wheat Purchase #', id) as transaction, wheat_price as amount from wheat_purchase where supplier_id = '".$supplier_id."' and datetime_added <='".$dt."' union select concat( 'Wheat Purchase Return #', id) as transaction, -wheat_price as amount from wheat_purchase_return where supplier_id = '".$supplier_id."' and datetime_added <='".$dt."' union select concat( 'Payment #', id) as transaction, -amount from supplier_payment where supplier_id = '".$supplier_id."' and datetime_added <='".$dt."') as transactions";
		$balance=dofetch(doquery($sql,$dblink));
