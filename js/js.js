		...
		$(".item_select").change(function(){
			...
			if( $container.length > 0 ) {
				...
				if (($('#transaction_id').length > 0)){
					...
					$transId=0;
				if($val>0){
					$.get("purchase_manage.php", {"tab":"get_purchase_price","id":$val ,'transcationid':$transId}, function($unit_price){
						$container.find(".unit_price").val($unit_price);
						update_total($container);
					});
				}
...
function update_total($container){
	...
	$container.find(".total_price").val(((parseFloat($container.find(".unit_price").val()))*parseFloat($container.find(".quantity").val())).toFixed(2));
