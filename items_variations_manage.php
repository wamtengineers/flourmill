...
define("APP_START", 1);
$filename = 'items_variations_manage.php';
if(isset($_GET["parent_id"])){
	$_SESSION["items_variations"]["item_id"]=$_GET["parent_id"];
}
if( isset( $_SESSION["items_variations"]["item_id"] ) ) {
	$parent_item_id = $_SESSION["items_variations"]["item_id"];
}
else {
	die( 'Select Parent Item Variations' );
}
$parent_item = doquery( "select * from items where id='".$parent_item_id."'", $dblink );
if( numrows( $parent_item ) ) {
	$parent_item = dofetch( $parent_item );
}
else{
 	die( "Parent Item does not exists" );
}
$tab_array=array("list", "add", "edit", "print", "delete", "bulk_action");
...
switch($tab){
	case 'add':
		include("modules/items_variations/add_do.php");
	break;
	case 'edit':
		include("modules/items_variations/edit_do.php");
	break;
	case 'print':
		include("modules/items_variations/print.php");
		die;
	break;
	case 'delete':
		include("modules/items_variations/delete_do.php");
	break;
	case 'bulk_action':
		include("modules/items_variations/bulkactions.php");
	break;
}
?>
            ...
            <div class="container-widget row">
                <div class="col-md-12">
                  <?php
                    switch($tab){
                        case 'list':
                            include("modules/items_variations/list.php");
                        break;
                        case 'add':
                            include("modules/items_variations/add.php");
                        break;
                        case 'edit':
                            include("modules/items_variations/edit.php");
                        break;
                    }
                  ?>
                </div>
