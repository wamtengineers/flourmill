...
if(isset($_POST["action"])){
	...
	switch($_POST["action"]){
		...
		case "get_addedit":
			...
			if( numrows( $rs ) > 0 ) {
				...
				if( !empty( $r[ "brokery_id" ] ) ) {
					$expense = doquery( "select * from expense where id = '".$r[ "brokery_id" ]."'", $dblink );
		...
		case "save_addedit":
			...
			if( count( $err ) == 0 ) {
				if( !empty( $addedit->id ) ) {
					$check = dofetch( doquery( "select cnf, fare_transaction_id from purchase_return where id = '".$addedit->id."'", $dblink ));
					...
					doquery( "insert into purchase_return (datetime_added, bill_no, account_id, less_weight, discount, status, cnf, added_by) VALUES ('".slash(datetime_dbconvert($addedit->datetime_added))."', '".slash($addedit->bill_no)."', '".slash($addedit->account_id)."', '".slash($addedit->less_weight)."', '".slash($addedit->discount)."', '".slash($addedit->status)."', '".slash($addedit->cnf)."', '".$_SESSION[ "logged_in_admin" ][ "id" ]."')", $dblink );
					$addedit_id = inserted_id();
				}
				if( !empty( $addedit->broker_id ) && !empty( $addedit->broker_amount ) ) {
					$update = false;
					if( !empty( $addedit->brokery_id ) ) {
						$expense = doquery( "select id from expense where id='".$addedit->brokery_id."'", $dblink );
						if( numrows( $expense ) > 0 ) {
							$update = true;
						}
					}
					if( $update ) {
						doquery( "update expense set datetime_added='".slash(datetime_dbconvert($addedit->datetime_added))."', account_id = '".slash( $addedit->broker_id )."', amount = '".slash( $addedit->broker_amount )."' where id = '".$addedit->brokery_id."'", $dblink );
					}
					else {
						doquery( "insert into expense(datetime_added, expense_category_id, account_id, amount, details, added_by) values( '".slash(datetime_dbconvert($addedit->datetime_added))."', '".get_config( "brokery_category_id" )."', '".slash( $addedit->broker_id )."', '".slash( $addedit->broker_amount )."', 'Brokery against Purchase Return #".$addedit_id."', '".$_SESSION[ "logged_in_admin" ][ "id" ]."' )", $dblink );
						$addedit->brokery_id = inserted_id();
					}
				}
				else{
					if( !empty( $addedit->brokery_id ) ) {
						doquery( "delete from expense where id = '".$addedit->brokery_id."'", $dblink );
						$addedit->brokery_id = 0;
					}
				}
				doquery( "update purchase_return set brokery_id = '".$addedit->brokery_id."' where id='".$addedit_id."'", $dblink );
				if( !empty( $addedit->fare_of_vehicle_payment_account_id ) && !empty( $addedit->fare_of_vehicle ) ) {
					if( $addedit->cnf==1 ){
						...
						if( $update ) {
							...
							doquery( "insert into expense(datetime_added, expense_category_id, account_id, amount, details, added_by) values( '".slash(datetime_dbconvert($addedit->datetime_added))."', '".get_config( "fare_category_id" )."', '".slash( $addedit->fare_of_vehicle_payment_account_id )."', '".slash( $addedit->fare_of_vehicle )."', 'Fare of Vehicle against Purchase Return #".$addedit_id."', '".$_SESSION[ "logged_in_admin" ][ "id" ]."' )", $dblink );
				...
				doquery( "update purchase_return set fare_transaction_id = '".$addedit->fare_transaction_id."' where id='".$addedit_id."'", $dblink );
				if( !empty( $addedit->payment_account_id ) && !empty( $addedit->payment_amount ) ) {
					...
					if( $update ) {
						doquery( "update transaction set datetime_added='".slash(datetime_dbconvert($addedit->datetime_added))."', account_id = '".slash( $addedit->payment_account_id )."', amount = '".slash( $addedit->payment_amount )."', reference_id = '".slash( $addedit->account_id )."' where id = '".$addedit->transaction_id."'", $dblink );
					}
					else {
						doquery( "insert into transaction(datetime_added, account_id, amount, reference_id, details, added_by) values( '".slash(datetime_dbconvert($addedit->datetime_added))."', '".slash( $addedit->payment_account_id )."', '".slash( $addedit->payment_amount )."', '".slash( $addedit->account_id )."', 'Payment against Purchase Return #".$addedit_id."', '".$_SESSION[ "logged_in_admin" ][ "id" ]."' )", $dblink );
				...
				foreach( $addedit->items as $item ) {
					if( !empty( $item->id ) ) { 
						doquery( "update purchase_return_items set `item_id`='".slash( $item->item_id )."', `packing`='".slash( $item->packing )."', `unit_price`='".$item->unit_price."', `rate`='".$item->rate."', `quantity`='".$item->quantity."', `less_weight`='".$item->less_weight."', `total_price`='".$item->total."' where id='".$item->id."'", $dblink );
						$item_ids[] = $item->id;
					}
					else {						
						doquery( "insert into purchase_return_items ( purchase_return_id, item_id, packing, unit_price, rate, quantity, less_weight, total_price ) values( '".$addedit_id."', '".$item->item_id."', '".$item->packing."', '".$item->unit_price."', '".$item->rate."', '".$item->quantity."', '".$item->less_weight."', '".$item->total."' )", $dblink );
						$item->id = inserted_id();
						$item_ids[] = $item->id;
