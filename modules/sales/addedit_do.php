...
if(isset($_POST["action"])){
	...
	switch($_POST["action"]){
		...
		case "save_addedit":
			...
			if( count( $err ) == 0 ) {
				if( !empty( $addedit->id ) ) {
					$check = dofetch( doquery( "select cnf, fare_transaction_id from sales where id = '".$addedit->id."'", $dblink ));
				...
				doquery( "update sales set brokery_id = '".$addedit->brokery_id."' where id='".$addedit_id."'", $dblink );
				if( !empty( $addedit->fare_of_vehicle_payment_account_id ) && !empty( $addedit->fare_of_vehicle ) ) {
					if( $addedit->cnf==1 ){
						...
						if( $update ) {
							...
							doquery( "insert into expense(datetime_added, expense_category_id, account_id, amount, details, added_by) values( '".slash(datetime_dbconvert($addedit->datetime_added))."', '".get_config( "fare_category_id" )."', '".slash( $addedit->fare_of_vehicle_payment_account_id )."', '".slash( $addedit->fare_of_vehicle )."', 'Fare of Vehicle against Sales #".$addedit_id."', '".$_SESSION[ "logged_in_admin" ][ "id" ]."' )", $dblink );
				...
				doquery( "update sales set fare_transaction_id = '".$addedit->fare_transaction_id."' where id='".$addedit_id."'", $dblink );
				...
				doquery( "update sales set transaction_id = '".$addedit->transaction_id."' where id='".$addedit_id."'", $dblink );
				$item_ids = array();
				foreach( $addedit->items as $item ) {
					if( !empty( $item->id ) ) { 
						doquery( "update sales_items set `item_id`='".slash( $item->item_id )."', `packing`='".slash( $item->packing )."', `unit_price`='".$item->unit_price."', `rate`='".$item->rate."', `quantity`='".$item->quantity."', `less_weight`='".$item->less_weight."', `total_price`='".$item->total."' where id='".$item->id."'", $dblink );
						$item_ids[] = $item->id;
					}
					else {						
						doquery( "insert into sales_items ( sales_id, item_id, packing, unit_price, rate, quantity, less_weight, total_price ) values( '".$addedit_id."', '".$item->item_id."', '".$item->packing."', '".$item->unit_price."', '".$item->rate."', '".$item->quantity."', '".$item->less_weight."', '".$item->total."' )", $dblink );
						$item->id = inserted_id();
						$item_ids[] = $item->id;
