...
if(isset($_POST["action"])){
	...
	switch($_POST["action"]){
		...
		case "get_addedit":
			$id = slash( $_POST[ "id" ] );
			$rs = doquery( "select * from purchase where id='".$id."'", $dblink );
			if( numrows( $rs ) > 0 ) {
				$r = dofetch( $rs );
				$addedit = array(
					"id" => $r[ "id" ],
		...
		case "save_addedit":
			...
			if( count( $err ) == 0 ) {
				if( !empty( $addedit->id ) ) {
					$check = dofetch( doquery( "select cnf, fare_transaction_id from purchase where id = '".$addedit->id."'", $dblink ));
				...
				if( !empty( $addedit->broker_id ) && !empty( $addedit->broker_amount ) ) {
					...
					if( $update ) {
						...
						doquery( "insert into transaction(datetime_added, account_id, reference_id, amount, details, added_by) values( '".slash(datetime_dbconvert($addedit->datetime_added))."', '".$addedit->account_id."', '".slash( $addedit->broker_id )."', '".slash( $addedit->broker_amount )."', 'Brokery against Purchase #".$addedit_id."', '".$_SESSION[ "logged_in_admin" ][ "id" ]."' )", $dblink );
				...
				doquery( "update purchase set brokery_id = '".$addedit->brokery_id."' where id='".$addedit_id."'", $dblink );
				...
				doquery( "update purchase set fare_transaction_id = '".$addedit->fare_transaction_id."' where id='".$addedit_id."'", $dblink );
				if( !empty( $addedit->payment_account_id ) && !empty( $addedit->payment_amount ) ) {
					...
					if( $update ) {
						...
						doquery( "insert into transaction(account_id, datetime_added, amount, reference_id, details, added_by) values( '".slash( $addedit->account_id )."', NOW(), '".slash( $addedit->payment_amount )."', '".slash( $addedit->payment_account_id )."', 'Payment against Purchase #".$addedit_id."', '".$_SESSION[ "logged_in_admin" ][ "id" ]."' )", $dblink );
				...
				doquery( "update purchase set transaction_id = '".$addedit->transaction_id."' where id='".$addedit_id."'", $dblink );
				$item_ids = array();
				foreach( $addedit->items as $item ) {
					if( !empty( $item->id ) ) { 
						doquery( "update purchase_items set `item_id`='".slash( $item->item_id )."', `packing`='".slash( $item->packing )."', `unit_price`='".$item->unit_price."', `rate`='".$item->rate."', `quantity`='".$item->quantity."', `less_weight`='".$item->less_weight."', `total_price`='".$item->total."' where id='".$item->id."'", $dblink );
						$item_ids[] = $item->id;
					}
					else {						
						doquery( "insert into purchase_items ( purchase_id, item_id, packing, unit_price,  rate, quantity, less_weight, total_price ) values( '".$addedit_id."', '".$item->item_id."', '".$item->packing."', '".$item->unit_price."', '".$item->rate."', '".$item->quantity."', '".$item->less_weight."', '".$item->total."' )", $dblink );
						$item->id = inserted_id();
						$item_ids[] = $item->id;
				...
				if( !empty( $addedit->id ) && count( $item_ids ) > 0 ) {
					$rs = doquery( "select * from purchase_items where purchase_id='".$addedit_id."' and id not in( ".implode( ",", $item_ids )." )", $dblink );
					
					doquery( "delete from purchase_items where purchase_id='".$addedit_id."' and id not in( ".implode( ",", $item_ids )." )", $dblink );
				}
				$response = array(
					"status" => 1,
					"id" => $addedit_id
