...
if(isset($_GET["account_type_id"])){
	$account_type_id=slash($_GET["account_type_id"]);
	$_SESSION["account"]["list"]["account_type_id"]=$account_type_id;
}
if(isset($_SESSION["account"]["list"]["account_type_id"]))
	$account_type_id=$_SESSION["account"]["list"]["account_type_id"];
else
	$account_type_id="";
if($account_type_id!=""){
	$extra.=" and account_type_id='".$account_type_id."'";
	$is_search=true;
}
?>
    ...
    <li class="col-xs-12 col-lg-12 col-sm-12">
    	<div>
        	<form class="form-horizontal" action="" method="get">
                ...
                <div class="col-sm-3">
                	<select name="account_type_id" id="account_type_id" title="Choose Option">
                        <option value="">Select Account Type</option>
                        <?php
                        $rs = doquery( "select * from account_type order by sortorder", $dblink );
						if( numrows( $rs ) > 0 ) {
							while( $r = dofetch( $rs ) ) {
								?>
								<option value="<?php echo $r[ "id" ]?>"<?php echo $account_type_id == $r[ "id" ]?' selected':''?>><?php echo unslash( $r[ "title" ] )?></option>
								<?php
							}
						}
						?>
...
<div class="panel-body table-responsive">
	<table class="table table-hover list">
        ...
        <tbody>
            <?php 
            $sql="select * from account where 1 $extra order by id";
            ...
            if(numrows($rs)>0){
                ...
                while($r=dofetch($rs)){             
                    ...
                    <tr>
                        ...
                        <td><?php echo get_field($r["account_type_id"], "account_type","title"); ?></td>
