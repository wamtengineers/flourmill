...
<div id="main">
    ...
    <div class="contentbox">
        ...
        <table cellpadding="0" cellspacing="0" align="center" width="800" border="0" class="items">
            ...
            <?php
            $items=doquery("select a.*, b.title, c.title as category from sales_items a left join items b on a.item_id=b.id left join item_category c on b.item_category_id = c.id where sales_id='".$sale["id"]."' order by a.id", $dblink);
			$total_discount = 0;
            if(numrows($items)>0){
                $sn=1;
                while($item=dofetch($items)){
                    ?>
                    <tr>
                    	<td style="text-align:left;"><?php echo $item["quantity"]?> &times; <?php echo unslash($item["title"])." ".unslash($item["category"])?>
                        	<?php
