...
if(isset($_GET["action"]) && $_GET["action"]!=""){
	...
	if(empty($err)){
		if($bulk_action=="delete"){
			$i=0;
			while($i<count($id)){
				doquery("delete from items_attributes where id='".$id[$i]."' and item_id='".$parent_item_id."'",$dblink);
				$i++;
			}
		...
		header("Location: items_attributes_manage.php?tab=list&err=".url_encode($err));
		die;					
	}
}
else{
