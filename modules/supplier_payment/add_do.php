...
if(isset($_POST["supplier_payment_add"])){
	...
	if($err==""){
		$sql="INSERT INTO supplier_payment (supplier_id, datetime_added, amount, account_id, details) VALUES ('".slash($supplier_id)."','".slash(datetime_dbconvert($datetime_added))."','".slash($amount)."','".slash($account_id)."','".slash($details)."')";
		doquery($sql,$dblink);
